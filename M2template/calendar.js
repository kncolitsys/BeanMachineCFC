...
function showCalendar(dateField, isFutureAllowedLocal) {
    if (isFutureAllowedLocal == 'true')
        isFutureAllowed = true;
    else
        isFutureAllowed = false;
    //alert("inside show calendar");
    // SET INITIAL VALUE OF THE DATE FIELD AND CREATE TOP AND BOTTOM FRAMES
    setDateField(dateField);
    //alert("date field set");
    ...
    calDocFrameset = 
		...
		"	parent.opener.top.newWin = null;\n" +
		"}\n</script>\n" +
		"</HEAD>\n" +
        "<FRAMESET ROWS='28%,*' FRAMEBORDER='1' onUnload='finishOff()'>\n" +
        "  <FRAME NAME='topCalFrame' SRC='javascript:parent.opener.calDocTop' noresize SCROLLING='no'>\n" +
	...
	if(top.newWin == null) {
	    top.newWin = window.open("javascript:parent.opener.calDocFrameset", "calWin", 'dependent=yes, width=210,   height=225, screenX=200, screenY=300, titlebar=yes');
	}
	//newWin.focus();
}
...
function buildTopCalFrame() {
    ...
    var calDoc =
        ...
        "<a href= 'javascript: parent.opener.returnToday()'> Today </a>" +
        "</TD>" +
        "</TR>" +
        "</TABLE>" +
        "</CENTER>" +
...
function setCurrentYear() {
    ...
    // GET THE NEWLY SELECTED YEAR AND CHANGE THE CALENDAR ACCORDINGLY
    var index = top.newWin.frames['topCalFrame'].document.calControl.year.selectedIndex;
    var year = (top.newWin.frames['topCalFrame'].document.calControl.year.options[index]).value;
...
// BUILD THE YEAR SELECT LIST
function getYearSelect() {

    // BROWSER LANGUAGE CHECK DONE PREVIOUSLY (navigator.language())

	/*
    yearArray = new Array('2003', '2004', '2005', '2006', '2007', '2008',
                           '2009', '2010', '2011', '2012', '2013', '2014');
	*/

	// DETERMINE YEAR TO SET AS DEFAULT
    var activeYear = new Date().getFullYear();

	//SPR 15211 fix.. Default Year is always taken as current year..
	//Fixed to set default year as the year in the date field if any date is entered.
	var selectedYear = activeYear;
	if(calDate != null) {
		selectedYear = calDate.getFullYear();
	}

	// Generate Year Drop Down List based on current system date.
	yearArray = new Array(noOfYears + 2);
	//alert(noOfYears);
	for(var j = 0; j < noOfYears; j++ ) {
		yearArray[j] = activeYear - 1 + j;
	}
    ...
    for (i in yearArray) {
		...
		// SHOW THE CORRECT YEAR IN THE SELECT LIST
		// if (i == activeYear) {
		if (yearArray[i] == selectedYear) {
            yearSelect += "<OPTION SELECTED value='" + yearArray[i] + "'>" + yearArray[i] + "\n";
        }
        else {
            yearSelect += "<OPTION value='" + yearArray[i] + "'>" + yearArray[i] + "\n";
        }
    }
    yearSelect += "</SELECT>";
...
//CHECK IF THE SELECTED IS LESS THAN CURRENT DATE
function isValidDate(inDate)
{
    ...
    //Selected Date
    var selDate = new Date(inDate);
    var selDay           = selDate.getDate();
    var selMonth         = selDate.getMonth()+1;
    var selYear          = selDate.getFullYear();

    //Fix for SPR 10248
    /*if( (selYear > year) || ((selYear == year) && (selMonth > month)) || ((selYear == year) && (selMonth == month) && (selDay >= day)))
    {
        if (isFutureAllowed)
            return true;
        else {
            alert("Please select a date less than or same as current date");
            newWin.focus();
            return false;
        }
    }
    ...
    {
        if (isFutureAllowed) {
            alert("Please select a date greater than or same as current date");
            newWin.focus();
            return false;
        } else return true;
    }
    }*/
    //Fix for accepting pastdate+currentdate and currentdate+futuredate in different scenarios
    if(isFutureAllowed)
    {
...
function returnDate(inDay)
{
    ...
    //IS SELECTED DATE LESS THAN CURRENT DATE
